...
try:
    ...
    from .crawler.ted import TEDCrawler
    from .crawler.ungm import UNGMCrawler
except ImportError:
    from database.storage import Storage, BidInfo
    from matcher.keyword import KeywordMatcher
    from notifier.email import EmailNotifier
...
def get_all_crawlers():
    ...
    注意：只有在这里注册的站点才会用专用爬虫；未注册的会退化成
    "抓首页链接"的通用爬虫，实测对绝大多数站点无效。
    """
    return {
...
def get_default_sites():
    """获取默认的内置网站列表"""
    return {
        'ggzy': {'name': '全国公共资源交易平台', 'url': 'https://www.ggzy.gov.cn/'},
        'ted': {'name': 'TED 欧盟标讯', 'url': 'https://ted.europa.eu/'},
        'ungm': {'name': 'UNGM 联合国采购', 'url': 'https://www.ungm.org/Public/Notice'},
